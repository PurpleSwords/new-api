...
func TestApplyWebSocketSubprotocolAuthorizationDoesNotOverrideProtocolOnly(t *testing.T) {
	...
	if applyWebSocketSubprotocolAuthorization(request) {
		t.Fatal("authorization was unexpectedly applied")
	}
	if got := request.Header.Get("Authorization"); got != "Bearer sk-original" {
...
func TestApplyWebSocketSubprotocolAuthorizationOverridesWithInsecureKey(t *testing.T) {
	...
	if got := request.Header.Get("Authorization"); got != "Bearer sk-from-protocol" {
		t.Fatalf("Authorization = %q, want protocol bearer", got)
	}
}

func TestApplyWebSocketSubprotocolAuthorizationIgnoresHTTPRequests(t *testing.T) {
	request := httptest.NewRequest(http.MethodPost, "/v1/responses", nil)
	request.Header.Set("Authorization", "Bearer sk-original")
	request.Header.Set("Sec-WebSocket-Protocol", "responses, openai-insecure-api-key.sk-from-protocol")

	if applyWebSocketSubprotocolAuthorization(request) {
		t.Fatal("authorization was unexpectedly applied to a non-websocket request")
	}
	if got := request.Header.Get("Authorization"); got != "Bearer sk-original" {
		t.Fatalf("Authorization = %q, want original bearer", got)
	}
