...
test('applies local filters to logs and statistics without changing the URL', async () => {
  const user = userEvent.setup()
  const { client, router } = await renderLogs({
    role: ROLE.USER,
    enabledPlans: [],
    activeSubscription: false,
  })
  ...
  await waitFor(() =>
    expect(api.get).toHaveBeenCalledWith(
      expect.stringMatching(/^\/api\/log\/self\?.*model_name=custom-model/)
    )
  )
  expect(api.get).toHaveBeenCalledWith(
    expect.stringMatching(/^\/api\/log\/self\/stat\?.*model_name=custom-model/)
  )
